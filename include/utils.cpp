...
std::string Utils::BinToBase64(const std::vector<unsigned char> &message)
{
...
std::vector<unsigned char> Utils::Base64ToBin(const std::string &message)
{
...
const std::vector<unsigned char> Utils::Enc(const std::vector<unsigned char> &plain_master_key, const std::vector<unsigned char> &salt)
{
    if (sodium_init() < 0)
    {
...
bool Utils::validate_master_key(const std::vector<unsigned char> &hashed_master_key, const std::vector<unsigned char> &dummy, const std::vector<unsigned char> &dummyNonce)
{
    if (sodium_init() < 0)
    {
        std::cerr << "Failed to initialize libsodium \n";
